...
static void _js_on_enter(eos_activity_t *activity)
{
    ...
    script_engine_result_t ret = script_engine_run(&self->data.js.pkg);
    if (ret != SE_OK) {
...
static void _js_on_resume(eos_activity_t *activity)
{
    ...
    script_engine_result_t ret = script_engine_watchface_resume(self->id);
    if (ret != SE_OK) {
        EOS_LOG_W("JS watchface resume failed for %s", self->id);
    }
