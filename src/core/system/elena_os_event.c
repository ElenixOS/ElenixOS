    ...
    while (*curr)
    {
        if ((*curr)->marked_for_delete)
        {
            ...
            *curr = (*curr)->next;
            ...
            /* 如果在未由 LVGL 删除对象时需要移除 LVGL 回调，可在此处调用 lv_obj_remove_event_cb
             * 但注意：如果对象已经被 lv_obj_del()，LVGL 已经移除了回调。
             * 对于手动调用 eos_event_remove_cb 的场景，lv_obj_remove_event_cb 已在调用点处理。
             */
            lv_free(tmp);
            g_event_list_modified = true; /* 确保状态一致 */
        }
        else
        {
            curr = &(*curr)->next;
...
/* ----- _obj_delete_cb: 当 LVGL 对象被删除时触发（LV_EVENT_DELETE） ----- */
/* 这里我们不直接 free 节点（如果正在广播），而是标记删除；若未在广播中，立即清理 */
...
void eos_event_init(void)
{
    ...
    for (uint32_t i = 0; i < EOS_EVENT_MAX_NUMBER; i++)
    {
        ...
        EOS_LOG_I("Event code registed | EOS[%d] -> LVGL[%d]", i, event_list[i]);
    }
}
